...
func BuildSynthesisPrompt(input *review.ReviewInput, results []*review.ModelReviewResult) string {
	...
	b.WriteString("2. Verify each finding against the diff: keep it only if you can name a concrete failure scenario (specific inputs, state, or sequence -> wrong result or crash) grounded in the changed code. Drop weak, speculative, hallucinated, or ungrounded findings.\n")
	...
	b.WriteString("A finding reported by only one reviewer is not automatically suspect: if it holds up against the diff, keep it. Treat agreement between reviewers as a signal that raises confidence, never as a requirement for inclusion.\n\n")
